...
public class StrengthTestScreen {
		...
		if (startTest_ImageButton.getIsClicked()) {
			if (allConnectedNodes != null && allConnectedNodes.size() > 0) {
				strengthTestThread = new Thread(new Runnable() {
					@Override
					public void run() {
						startedTest = !startedTest;
						// p.println("startedTest: ",startedTest);
						controllStrengthTest(startedTest);
					}
				});
	...
	private void checkForFinished() {
		...
		p.println(respondingPCs.size(), finishedPCs.size());
	...
	/**
	 * Function to reset all strengthTestStatus parameter in SystemLog files back to
	 * -1 --> If the user restarts the test, it wont be already finished.
	 */
	private void resetPCStatusInLogFiles(ArrayList<Node> respondingNodes) {
		for (int i = 0; i < respondingNodes.size(); i++) {
			Node n = respondingNodes.get(i);
			JSONArray loadedData = new JSONArray();
			JSONObject settingsDetails = new JSONObject();
			JSONObject settingsObject = new JSONObject();
			// give command to all pcs to do test -----------------------------
			String jsonPath = mainActivity.getPathToPCFolder() + "\\" + n.getPcSelection_DropdownMenu().getSelectedItem() + "\\" + mainActivity.getLogFileName();
			loadedData = jsonHelper.getData(jsonPath);

			if (loadedData.isEmpty()) {
			} else {
				try {
					p.println("reset pcstatus");
					JSONObject loadedObject = (JSONObject) (loadedData.get(0));
					loadedObject = (JSONObject) loadedObject.get("SystemLog");
					loadedObject.put("strengthTestStatus", -1);
					settingsObject.put("SystemLog", loadedObject);
					loadedData.set(0, settingsObject);
					jsonHelper.setArray(loadedData);
	...
	public void onScroll(float e) {
		strengthTest_HorizontalList.onScroll(e);
	}
