...
public class HomeScreen {
	...
	public HomeScreen(PApplet p, int btnSize, int btnSizeSmall, int edgeRad, int margin, int stdTs, int dark, int light, int lighter, int border, int textCol, int textDark, float textYShift, String[] homeScreenPictoPaths, String[] arrowPaths, String[] hoLiPictoPaths, String[] fileExplorerPaths, PFont stdFont) {
		...
		for (int i = 0; i < homeSettings_checkboxes.length; i++) {
			int ys = +mainButtons[0].getH() / 2;
			int is = 0;
			if (i > 3) {
				ys = p.height / 5 + mainButtons[0].getH() / 2;
				is = 4;
			}
			homeSettings_checkboxes[i] = new Checkbox(p, (int) (p.width / 9 * 1.5f + (p.width / 9 * 2) * (i - is)), p.height / 5 * 2 + ys, p.width / 9, btnSizeSmall, btnSizeSmall, edgeRad, margin, stdTs, light, light, border, textCol, textYShift, false, false, checkBoxTexts[i], checkBoxHoverTexts[i], homeScreenPictoPaths[0], stdFont, null);
		}
		...
		startRendering_btn = new ImageButton(p, p.width / 2, p.height / 5 * 4 + mainButtons[0].getH() / 2, btnSize, btnSize, stdTs, margin, edgeRad, -1, textYShift, true, false, textCol, light, homeScreenPictoPaths[2], "Start rendering", null);
