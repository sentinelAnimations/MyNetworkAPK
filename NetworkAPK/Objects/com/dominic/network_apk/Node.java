...
public class Node<T> {
	...
	private int x, y, dragShiftX, dragShiftY, headY, bodyY, w, h, bodyH, headH, type, edgeRad, margin, stdTs, btnSizeSmall, dark, darkest, bgCol, textCol, textDark, lighter, lightest, border, red, doOnce = 0, anzTypes = 5, conS, prevPortCount = 0, cpuCores;
	...
	public void checkForSignsOfLife() {
		...
		if (curTime - lastLogTime > 10) {
			setIsReady(comparePCLogData());
			lastLogTime = System.nanoTime() / 1000000000;
		}
	}

	private Boolean comparePCLogData() {
		...
		if (pcAlias.length() > 0) {
			...
			if (loadedSettingsData.isEmpty()) {
				...
				p.println(logTime, curTime, curTime - logTime, "---");
				if (curTime - logTime > mainActivity.getStdLogTimeIntervall()) {
					pcIsAlive = false;
				}
			}
		} else {
	...
	public void setCpuText(String cpuT) {
		cpuText = cpuT;
	}
